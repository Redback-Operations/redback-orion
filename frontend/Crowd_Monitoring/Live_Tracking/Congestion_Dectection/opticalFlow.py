...
class OpticalFlow:
    ...
    def drawPredictedTrajectory(self, frame, flow):
        # Function to draw the predicted trajectory on the frame
        ...
        try:
            ...
            # Make sure fx and fy have the same shape and are float32
            if fx.shape != fy.shape:
                print("Flow components have mismatched shapes")
                return frame.copy()
            ...
            bgr = cv2.addWeighted(frame, 0.5, bgr, 0.5, 0)
            bgr = cv2.resize(bgr, (frame.shape[1], frame.shape[0]))
            return bgr

        except Exception as e:
            print(f"Error in drawPredictedTrajectory: {e}")
            return frame.copy()  # Return original frame if any error occurs
