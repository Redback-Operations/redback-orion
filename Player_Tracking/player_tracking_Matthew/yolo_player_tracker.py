...
def classify_team(features):
    ...
    # Hardcoded umpire detection first
    if (
        yellow_ratio > 0.28 and
        red_ratio < 0.12 and
        median_s > 50 and
        median_v > 70
    ):
...
while True:
    ...
    if boxes is not None and len(boxes) > 0:
        ...
        if not clustering_ready and len(colour_feature_samples) >= MIN_CLUSTER_SAMPLES:
        ...
        for i in range(len(boxes)):
            ...
            if raw_team_label == "Umpire":
                umpire_streak[track_id] += 1
            else:
                umpire_streak[track_id] = max(0, umpire_streak[track_id] - 2)

            if umpire_streak[track_id] >= 10:
                team_label = "Umpire"
            else:
                team_label = raw_team_label
            ...
            if team_label == "Team_1":
                box_colour = (255, 255, 255)
                text_colour = (0, 0, 0)
                label_bg_colour = (255, 255, 255)

            elif team_label == "Team_2":
                box_colour = (0, 0, 0)
                text_colour = (255, 255, 255)
                label_bg_colour = (0, 0, 0)

            elif team_label == "Umpire":
                box_colour = (0, 255, 255)
                text_colour = (0, 0, 0)
                label_bg_colour = (0, 255, 255)

            elif team_label == "Learning":
                box_colour = (128, 128, 128)
                text_colour = (255, 255, 255)
                label_bg_colour = (128, 128, 128)
