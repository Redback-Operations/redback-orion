      ...
      setPlayers(prevPlayers => 
        prevPlayers.map(player => ({
          ...
          stats: {
            ...
            efficiency: Math.max(60, Math.min(95, player.stats.efficiency + (Math.random() - 0.5) * 2))
          }
        }))
      );
    }, 5000);
      ...
      <div className="space-y-2">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="text-xs w-16 truncate">{player1.name.split(' ')[0]}</span>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs w-16 truncate">{player2.name.split(' ')[0]}</span>
            <div className="flex-1">
              <Progress value={(val2 / max) * 100} className="h-2 bg-orange-100" />
            </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="text-xl">{selectedPlayer.name}</CardTitle>
              ...
              <Tabs defaultValue="stats" className="w-full">
                ...
                <TabsContent value="stats" className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    <StatCard title="Disposals" value={selectedPlayer.stats.disposals} color="blue" trend="up" />
                    <StatCard title="Kicks" value={selectedPlayer.stats.kicks} color="green" />
                    <StatCard title="Handballs" value={selectedPlayer.stats.handballs} color="purple" />
                    <StatCard title="Marks" value={selectedPlayer.stats.marks} color="orange" />
                    <StatCard title="Tackles" value={selectedPlayer.stats.tackles} color="red" />
                    <StatCard title="Goals" value={selectedPlayer.stats.goals} color="yellow" />
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{selectedPlayer.stats.efficiency}%</span>
                        </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">{selectedPlayer.stats.goalAccuracy}%</span>
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                      <div className="p-2 bg-gray-50 rounded">
                        <div className="text-lg font-semibold">{selectedPlayer.stats.contested}</div>
                        <div className="text-xs text-gray-600">Contested</div>
                      </div>
                      <div className="p-2 bg-gray-50 rounded">
                        <div className="text-lg font-semibold">{selectedPlayer.stats.uncontested}</div>
                        <div className="text-xs text-gray-600">Uncontested</div>
                      </div>
                      <div className="p-2 bg-gray-50 rounded">
                        <div className="text-lg font-semibold">{selectedPlayer.stats.inside50s}</div>
                        <div className="text-xs text-gray-600">Inside 50s</div>
                      </div>
                      <div className="p-2 bg-gray-50 rounded">
                        <div className="text-lg font-semibold">{selectedPlayer.stats.clangers}</div>
                    ...
                    <div className="grid grid-cols-7 gap-1">
                      {selectedPlayer.form.map((score, index) => (
                        <div key={index} className="text-center">
                          ...
                          <div className="text-xs text-gray-500 mt-1">R{7-index}</div>
                        </div>
                      ))}
                    </div>
                    <div className="text-sm text-gray-600">
                      Average: {(selectedPlayer.form.reduce((a, b) => a + b, 0) / selectedPlayer.form.length).toFixed(1)}
                    </div>
                  </div>
                </TabsContent>
                      ...
                      <div key={index} className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-gray-500 mt-1">Touches</div>
                          </div>
                          <div className="flex-1">
                ...
                <TabsContent value="compare" className="space-y-4">
                  <div className="space-y-3">
                    ...
                    <div className="space-y-4">
                      {['disposals', 'kicks', 'handballs', 'marks', 'tackles', 'goals', 'efficiency'].map((stat) => (
                        <ComparisonChart 
                          key={stat}
                          stat={stat}
